...
def test_ALTER_TABLE_MODIFY_COLUMN():
    query = """ALTER TABLE DWH_SFVR.VREQR.VEQDR MODIFY column AUM_GROUP_TYPE TEXT(100) ;"""
    qp = QueryParser(logger, query)
    assert qp.query == query.lower()
    assert qp.query_type == 'ALTER TABLE'
...
def test_CREATE_MASKING_POLICY():
  ...
  RETURNS number ->
    ...
    assert qp.source_object == ['rd_wolf', 'rd_k3s_prod', 'sam_fund_admin_sensitive_number']
...
def test_CREATE_SESSION_POLICY():
    query = """CREATE TAG "DDS_CORE_DATA_PLATFORM"."PLATFORM_MONITORING"."BUSINESS_UNIT"""
    qp = QueryParser(logger, query)
    assert qp.query == query.lower()
...
def test_CREATE_TABLE():
    ...
    assert qp.source_object == ['dds_sam', 'dbt_cloud_pr_819_1104_intermediate', 'sust_field_type_correction']
...
FROM landing_scd.TMSDAT.businessclasslevel1
    ...
    assert qp.query == query.lower()
    assert qp.query_type == 'CREATE TABLE'
    assert qp.source_object == ['rd_scd', 'snapshots', 'scd_tmsdat__businessclasslevel1__snapshot__dbt_tmp']
...
def test_REMOVE_FILES():
    ...
    assert qp.query == query.lower()
    assert qp.query_type == 'REMOVE'
    assert qp.source_object == ['landing_wolf', 'public', 'attrep_is_landing_wolf_e1d8c1b1_0b28_264d_a1b4_53dc7af8d4e6/e1d8c1b1_0b28_264d_a1b4_53dc7af8d4e6/0/cdc00000229', 'csv']
...
def test_RESTORE():
    ...
    assert qp.source_object == ['internal_export_sam', 'ie_ads_staging', 'ie_ads_esg_portfolio_kpis__snapshot_new_hw']
